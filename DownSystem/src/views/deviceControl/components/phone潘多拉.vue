...
// const wsUrl = `Wss://cloud1788.com/ws/${getUserId()}_${Cookies.get(
//   "UUID"
// )}!!!w!88${route.query.command.deviceId}`;
// const wsUrl = "Ws://192.168.10.49:8888/ws";
const wsUrl = `Ws://38.180.188.66/ws/${getUserId()}_${Cookies.get(
  "UUID"
)}!!!w!88${JSON.parse(route.query.command).deviceId}`;
...
const baseParams = {
  terminal: 1,
  userId: JSON.parse(route.query.command).deviceId,
...
const canvasWidth =
  Math.floor(window.innerWidth * 0.15) < 360
    ? 360
    : Math.floor(window.innerWidth * 0.15);
...
const drawChilds = (ctx, parent) => {
  ...
  switch (className) {
    ...
    case "android.widget.AtoCompleteTextView":
      // case "android.widget.ImageView":
      drawRect(ctx, parent.boundsInScreen, "#DCDCDC", "");
      break;
    case "android.widget.Button":
      drawRect(ctx, parent.boundsInScreen, "#B22222", "");
      break;
    case "android.widget.TextView":
      drawRect(ctx, parent.boundsInScreen, "#F9F9F9", "");
      break;
    case "android.widget.CheckBox":
    case "android.widget.RadioButton":
    case "android.widget.Switch":
    case "android.widget.ToggleButton":
      drawRect(ctx, parent.boundsInScreen, "#FFFFFF", "");
      break;
    default:
      drawRect(ctx, parent.boundsInScreen, "#000000", "");
      break;
  }
  ...
  //绘制 childs
  let childList = parent.childs;
  if (childList != null && childList.length > 0) {
    let size = childList.length;
    for (let i = 0; i < size; i++) {
      let child1 = childList[i];
      // if (i + 1 < size) {
      //   let child2 = childList[i + 1];
      //   let rect1 = child1.boundsInScreen;
      //   let rect2 = child2.boundsInScreen;
...
const isAppIcon = (element) => {
  return (
    ("android.widget.RelativeLayout" && element.desc) ||
    ("android.widget.FrameLayout" && element.desc)
  );
};
...
const drawLayout = () => {
  ...
  rectangles = [];
  console.log("layoutData.nodeInfo", layoutData.nodeInfo);
...
const handleMouseUp = (event) => {
  ...
  phoneStore().setDynamicParams({
    ...baseParams,
    // terminal: 1, //终端类型1为管理终端  必传
    // userId: route.query.command.deviceId, //发给哪个终端 //必传
...
//       // userId: route.query.command.deviceId, // 发给哪个终端
//       // adminId: `${getUserId()}_${Cookies.get("UUID")}`,
//       // reqId: nanoid(), // 请求ID
//     };
