  ...
  <div ref="containerRef" class="canvas-container">
    ...
    >
      ...
      <div class="phone">
        ...
        <div class="nav-bar">
          <!-- 后台应用 -->
          <div class="nav-button back" @click="statusBarOrientation(3)">
            <span class="icon">□ </span>
          </div>
          <!-- 返回桌面 -->
          <div class="nav-button home" @click="statusBarOrientation(2)">
            <span class="icon">○</span>
          </div>
          <!-- 返回 -->
          <div class="nav-button recent" @click="statusBarOrientation(1)">
            <span class="icon">
...
import { uploadLayout } from "@/api/operatingApi/index";
const { t } = useI18n();
...
const isTestEnv = import.meta.env.VITE_USE_TEST_API === "true";
const protocol = isTestEnv ? "ws://" : "wss://";
const baseUrl = import.meta.env.VITE_API_URL;
const wsUrl = `${protocol}${baseUrl}/ws/${getUserId()}_${Cookies.get(
  "UUID"
)}!!!w!88${routeQuery.deviceId}`;

// const wsUrl = `Ws://38.180.188.66/ws/${getUserId()}_${Cookies.get(
//   "UUID"
// )}!!!w!88${routeQuery.deviceId}`;

// const wsUrl = `Ws://192.168.10.62:8080/ws/${getUserId()}_${Cookies.get(
//   "UUID"
// )}!!!w!88${route.query.deviceId}`;
...
//   return NettyMessage;
// };
// console.log("routeQuery.deviceId", routeQuery.deviceId);
...
let upErrorMessage = ref(false);
let isBackground = ref(false);

// 底部状态栏事件
const statusBarOrientation = (value) => {
...
const canvasWidth =
  Math.floor(window.innerWidth * 0.13) < 360
    ? 360
    : Math.floor(window.innerWidth * 0.23);
...
if (routeQuery.resolution) {
  [originalWidth, originalHeight] = routeQuery.resolution
    .split("*")
    .map(Number);
}
...
const CONFIG = {
  ...
  IMPORTANT_TYPES: new Set([
    ...
    "android.widget.Button",
    "android.widget.EditText",
    "android.widget.AtoCompleteTextView",
    "android.widget.RelativeLayout",
...
const drawChilds = (ctx, parent) => {
  // console.log("drawChilds", parent);

  // console.log("drawChilds", parent.boundsInScreen);
...
const processElements = (ctx, element) => {
  ...
  //争对优化三星密码输入键盘，但是跟vivo 应用绘制 冲突
  // if (
  //   element.className === "android.view.ViewGroup" &&
  //   element.desc &&
  ...
  //     ctx,
  //     element.boundsInScreen,
  //     getBackgroundColor(element.className),
  //     "",
  //     createTextStyle(scaleX, scaleY)
  //   );
  ...
  // 判断是否是键盘区域的父级布局
  const isKeyboardParentLayout = (element) => {
    return (
      element.className.includes("Layout") &&
      element.childs?.some(
        (child) =>
          child.className === "android.view.ViewGroup" &&
          child.boundsInScreen?.bottom > canvasHeight * 0.6
      )
    );
  };
  ...
  const textToShow = getElementText(element);
  let bgColor = null;
  if (!isKeyboardParentLayout(element)) {
    bgColor = getBackgroundColor(element.className);
  }
...
const hasNonLayoutChild = (element) => {
  if (!element.childs || element.childs.length === 0) {
    return false;
  }

  return element.childs.some(
    (child) => !child.className.includes("Layout") || hasNonLayoutChild(child)
  );
};
...
const isAppIcon = (element) => {
  return (
    (element.className === "android.widget.RelativeLayout" && element.desc) ||
    (element.className === "android.widget.LinearLayout" && element.desc) ||
    (element.className === "android.widget.FrameLayout" && element.desc)
  );
};
...
const drawAppIcon = (ctx, element) => {
  ...
  const bgColor = getBackgroundColor(element.className);
  drawRect(ctx, bounds, bgColor, text, iconTextStyle);
};
...
const getElementText = (element) => {
  if (element.className === "android.widget.RelativeLayout" && element.desc) {
    // console.log("titkon", element.desc);

    return element.desc;
  }
  if (CONFIG.IMPORTANT_TYPES.has(element.className)) {
    // console.log("titkon", element.desc, element.className);
...
const getBackgroundColor = (className) => {
  // CONFIG.IMPORTANT_TYPES.add("androidx.drawerlayout.widget.DrawerLayout");

  if (isBackground == true) {
    // console.log("我是关闭背景色", isBackground);
  ...
  if (!CONFIG.IMPORTANT_TYPES.has(className)) {
    // console.log("我是没有这个类", className);
  ...
  switch (className) {
    ...
    //   return "rgba(240, 248, 255, 2)";
    // case "android.widget.ScrollView":
    //   return "rgba(245, 245, 245, 2)";
...
watch(
  ...
  (newValue) => {
    ...
    if (newValue.isClose != undefined) {
      isBackground = newValue.isClose;
      // console.log("监听消息: ", newValue, isBackground);
    }
    if (newValue.isErrorMessage != undefined) {
      upErrorMessage.value = newValue.isErrorMessage;
      // console.log("监听消息: ", newValue, upErrorMessage.value);
    }
...
const initializeWebSocket = () => {
  ...
  socket.onopen = () => {
    ...
    socket.send(JSON.stringify(packType67));
  ...
  socket.onmessage = async (message) => {
    ...
    if (upErrorMessage.value) {
      // console.log("upErrorMessage", upErrorMessage.value);
      if (JSON.parse(message.data).nodeInfo != undefined) {
        // console.log("message.data", JSON.parse(message.data));
        upErrorMessage.value = false;
        uploadDeviceLayout(message.data);
      }
    }
    if (phoneStore().checkStaus.isSwitch) {
      // console.log("message.data", JSON.parse(message.data));

      handleWebSocketData(message.data);
    } else {
      // console.log("我是关闭黑屏助手", phoneStore().checkStaus.isSwitch);
...
const uploadDeviceLayout = (content) => {
  uploadLayout({
    content: content,
