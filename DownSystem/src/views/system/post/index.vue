...
const data = reactive({
  ...
  rules: {
    postName: [{ required: true, message: t('岗位名称')+t('不能为空'), trigger: "blur" }],
    postCode: [{ required: true, message: t('岗位编码')+t('不能为空'), trigger: "blur" }],
    postSort: [{ required: true, message:t('岗位顺序')+t('不能为空'), trigger: "blur" }],
  }
});
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm(t('是否确认删除岗位编号为') + postIds + t('的数据项？')).then(function() {
