...
const data = reactive({
  ...
  rules: {
    dictName: [{ required: true, message: t('字典名称')+t('不能为空'), trigger: "blur" }],
    dictType: [{ required: true, message: t('字典类型')+t('不能为空'), trigger: "blur" }]
  },
});
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm(t('是否确认删除字典编号为') + dictIds + t('的数据项？')).then(function() {
...
function handleRefreshCache() {
  refreshCache().then(() => {
    proxy.$modal.msgSuccess(t(t('刷新成功')));
