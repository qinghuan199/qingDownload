   ...
   <div class="app-container">
      <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
         ...
         <el-form-item :label="$t('系统内置')" prop="configType">
            <el-select v-model="queryParams.configType" :placeholder="$t('系统内置')" clearable>
               <el-option
      ...
      <el-dialog :title="title" v-model="open" width="500px" append-to-body>
         <el-form ref="configRef" :model="form" :rules="rules" label-width="80px">
            ...
            <el-form-item :label="$t('参数键值')" prop="configValue">
               <el-input v-model="form.configValue" :placeholder="$t('请输入参数键值')" />
            </el-form-item>
            <el-form-item :label="$t('系统内置')" prop="configType">
...
const data = reactive({
  ...
  rules: {
    configName: [{ required: true, message: t('参数名称')+t('不能为空'), trigger: "blur" }],
    configKey: [{ required: true, message:  t('参数键名')+t('不能为空'), trigger: "blur" }],
    configValue: [{ required: true, message:  t('参数键值')+t('不能为空'), trigger: "blur" }]
  }
});
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm(t('是否确认删除参数编号为') + configIds + t('的数据项？')).then(function () {
...
function handleRefreshCache() {
  refreshCache().then(() => {
    proxy.$modal.msgSuccess(t('刷新缓存成功'));
  });
}
