<template>
  <el-card>
    <el-tabs v-model="activeName">
      ...
      <el-tab-pane :label="$t('字段信息')" name="columnInfo">
        <el-table ref="dragTable" :data="columns" row-key="columnId" :max-height="tableHeight">
          <el-table-column :label="$t('序号')" type="index" min-width="5%"/>
          <el-table-column
            label="字段列名":label="$t('序号')"
            prop="columnName"
            min-width="10%"
          ...
          <el-table-column
            ...
            :show-overflow-tooltip="true"
          />
          <el-table-column :label="$t('Java类型')" min-width="11%">
            <template #default="scope">
          ...
          <el-table-column :label="$t('java属性')" min-width="10%">
            <template #default="scope">
              <el-input v-model="scope.row.javaField"></el-input>
            </template>
          </el-table-column>

          <el-table-column :label="$t('插入')" min-width="5%">
            <template #default="scope">
              <el-checkbox true-label="1" false-label="0" v-model="scope.row.isInsert"></el-checkbox>
            </template>
          </el-table-column>
          <el-table-column :label="$t('编辑')" min-width="5%">
            <template #default="scope">
              <el-checkbox true-label="1" false-label="0" v-model="scope.row.isEdit"></el-checkbox>
            </template>
          </el-table-column>
          <el-table-column :label="$t('列表')" min-width="5%">
            <template #default="scope">
              <el-checkbox true-label="1" false-label="0" v-model="scope.row.isList"></el-checkbox>
            </template>
          </el-table-column>
          <el-table-column :label="$t('查询')" min-width="5%">
            <template #default="scope">
          ...
          <el-table-column :label="$t('必填')" min-width="5%">
            <template #default="scope">
...
function submitForm() {
  ...
  Promise.all([basicForm, genForm].map(getFormPromise)).then(res => {
    ...
    if (validateResult) {
      ...
      proxy.$modal.msgError(t('表单校验未通过，请重新检查提交内容'));
    }
  });
}
