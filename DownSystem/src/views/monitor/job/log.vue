      ...
      <el-dialog title="调度日志详细" v-model="open" width="700px" append-to-body>
         <el-form :model="form" label-width="100px">
            <el-row>
               ...
               <el-col :span="12">
                  <el-form-item :label="$t('任务分组：')">{{ form.jobGroup }}</el-form-item>
                  <el-form-item :label="$t('执行时间：')">{{ form.createTime }}</el-form-item>
               </el-col>
               <el-col :span="24">
...
function handleDelete(row) {
  proxy.$modal.confirm(t('是否确认删除调度日志编号为') + ids.value + t('的数据项?')).then(function () {
...
function handleClean() {
  proxy.$modal.confirm(t('是否确认清空所有调度日志数据项?')).then(function () {
    return cleanJobLog();
  }).then(() => {
    getList();
