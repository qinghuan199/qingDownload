...
const loginRules = {
  ...
  verifyCode: [{ required: true, trigger: "change", message: t("请输入验证码") }],
  // code: [{ required: true, trigger: "change", message: t("请输入验证码") }],
};

const codeUrl = ref("");
const loading = ref(false);
const rootLogin = ref(false);
// 验证码开关
    ...
    if (valid) {
      ...
      userStore
        ...
        .then(() => {
          ...
          router.push({path: "/user1/userConfig" });
